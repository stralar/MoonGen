...
function forward(threadNumber, ns, ring, txQueue, txDev, rate, latency, xlatency, lossrate, clossrate, catchuprate,
	...
	-- the largest send_time of previous fames having concealed losses
			...
			for iix=1,count do
				...
				while (math.random() < clossrate) do
					...
					if (catchuprate > 0) then
						catchup_mode = true
						print("entering catchup mode!  ",closses)
					end
				end
				...
				-- When a frame has concealed losses it may cause a backup of
				-- of the frames behind it.  They have been received over the wireless
				-- link, but cannot be delivered out of order.  Once the concealed loss
				-- is corrected, these backed-up frames can be delivered immediately
				-- (or at a higher rate).  They have already made it through the
				-- wireless bottleneck.
				-- We need to record the latest intended send_time of all frames
				-- with concealed losses.  Frames backed up behind it with send_times
				-- that are earlier than cl_send_time can be delivered at the
				-- catchup_rate.
				-- NOTE: this ignores the passage of time /during/ the catch-up process.
				-- NOTE: it also doesn't properly account for the buffering of these
				--       frames at the receiver
				if (closses > 0) then
					if (send_time > cl_send_time) then
						cl_send_time = send_time
						print("new cl_send_time: ",cl_send_time)
					end
				elseif (send_time > cl_send_time) then
					-- the backed-up frames have been cleared
					if (catchup_mode) then
						print "exiting catchup mode!"
					end
					catchup_mode = false
				end
